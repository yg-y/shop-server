		...
		<view class="goods-list">
			<view class="product-list">
				<view class="product" v-for="product in productList" :key="product.id" @tap="toGoods(product)">
					<image mode="widthFix" :src="product.titlePhoto"></image>
					<view class="name">{{ product.name }}</view>
					<view class="info">
						<view class="price">{{ product.price }}</view>
						<view class="slogan">立即购买</view>
					</view>
				</view>
			</view>
		...
		data() {
			return {
				...
				productList: [{
					...
					price: 5499,
					titlePhoto: "https://www.apple.com/v/iphone-11/c/images/overview/design-gallery/design_gallery_3_purple__sixi1xr7f7ue_medium.jpg"
				}],
				loadingText: '正在加载...'
			};
		},
			...
			getProductList() {
				uni.request({
					...
					success: res => {
						...
						if(this.page.size > res.data.data.total){
